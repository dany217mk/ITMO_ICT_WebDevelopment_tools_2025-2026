...
def parse_github(soup, url):
    ...
    for repo in repos[:5]:
        try:
            ...
            print(f"  Ошибка парсинга репозитория: {e}")
            continue
...
def parse_habr(soup, url):
    ...
    for article in articles[:5]:
        try:
            ...
            print(f"  Ошибка парсинга статьи Habr: {e}")
            continue
    
    if count == 0:
        print(f" Habr: статей не найдено (возможно, нужны куки или JS)")
    ...
    for item in items[:5]:
        try:
            ...
            if title_elem:
                ...
                # Извлекаем теги/категории (по домену)
                domain = ""
                ...
                summary = f"Domain: {domain}" if domain else "Hacker News story"
                save_parsed_data(url, title, summary)
                print(f"  Hacker News: {title[:50]}")
                count += 1
        except Exception as e:
            print(f"  Ошибка парсинга Hacker News: {e}")
            continue
    
    return count

def parse_and_save(url, source_type):
    """Парсит страницу и сохраняет данные"""
    print(f"\n[THREADING] Начинаю парсинг: {url}")
...
def main():
    print("=" * 70)
    print("Запуск парсинга с использованием THREADING")
    print("=" * 70)
    ...
    for url, source_type in URLS:
        t = threading.Thread(target=parse_and_save, args=(url, source_type))
        threads.append(t)
        t.start()
    ...
    print("\n" + "=" * 70)
    print(f" THREADING завершен за {elapsed:.2f} секунд")
    get_stats()
    print("=" * 70)
